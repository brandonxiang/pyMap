...
def process_latlng(north, west, south, east, zoom):
    left, top = latlng2tilenum(north, west, zoom)
    right, bottom = latlng2tilenum(south, east, zoom)
    process_tilenum(left, right, top, bottom, zoom)


def process_tilenum(left, right, top, bottom, zoom):
    for x in range(left, right + 1):
        for y in range(top, bottom + 1):
            path = './tiles/%i/%i/%i' % (zoom, x, y)
            if not os.path.exists(path):
                download(x, y, zoom)
    mosaic(left, right, top, bottom, zoom)
...
def mosaic(left, right, top, bottom, zoom):
...
def cml():
    if(len(sys.argv)!=6):
        print('input 5 parameter northeast latitude,northeast longitude, southeast latitude, southeast longitude')
        return
